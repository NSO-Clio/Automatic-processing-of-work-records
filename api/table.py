...
class TableInformation:
    ...
    def get_data(self, img_path: str) -> dict:
        ...
        cv2.imwrite(f'temp{sep}elem.jpg', img[int(1652 * 0.26):, int(2232 * 0.086):int(2232 * 0.24)])

        # Распознавание текста на области интереса
        result = self.ocr_reader.doc2text(f'temp{sep}elem.jpg')
        data = {
            'dates': [], # data
            'records': [], # work
            'titles': [], # job_title
        ...
        for i, elem in enumerate(self.__calculate_y_distances(dt_box, img[int(1652 * 0.26):, :].shape[0])):
            ...
            cv2.imwrite(f'temp{sep}col_{i}.jpg', col_1)
            cv2.imwrite(f'temp{sep}col__2.jpg', col_2)
            result_1, result_2 = (self.ocr_reader.doc2text(f'temp{sep}col_{i}.jpg'),
                                  self.ocr_reader.doc2text(f'temp{sep}col__2.jpg'))
            data['dates'].append(text_dt[i] if len(text_dt) > i else "notFound")
            data['dates'].append(text_dt[i] if len(text_dt) > i else "notFound")
            data['titles'].append(
                result_1[0].lower().split('на должность')[-1].strip() if 'на должность' in ' '.join(
                    [self.russian.correction(i) if self.russian.correction(i) else i for i in
                     result_1[0].split()]).lower()
                else "notFound")
            data['records'].append(' '.join(
                [self.russian.correction(i) if self.russian.correction(i) else i for i in result_1[0].split()]))
            data['stamps'].append(' '.join(
                [self.russian.correction(i) if self.russian.correction(i) else i for i in result_2[0].split()]))
        return data
